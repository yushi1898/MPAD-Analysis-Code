...
//12/6/21 DHR started modifying for Igor9 wave refs
// 12/5/21  YS: moved plotting functions to PlotALot
...
variable pixratio=MicronsPerPixel
...
//currently in use
function calculateMSD(datawave, bgshiftwave, astrw, maskwave, framerate, spikeflag)
wave datawave
wave bgshiftwave
wave astrw
wave maskwave
...
make/O/N=(L-1, npost) MSD, MSD_ed
make/O/N=(L-1)  MSDxwave

variable spike_window = 10
variable iframe_left, iframe_right
...
for (ipost=0; ipost<npost; ipost+=1)
     ...
     if (spikeflag==1) 
             ...
             for (iframe=iframe1+1; iframe<iframe2; iframe+=1)  
             		iframe_left = max(iframe-spike_window,0)
             		iframe_right = min(iframe+spike_window,iframe2)
             		make/o/n=(iframe_right-iframe_left) dwtempx_short, dwtempy_short
             		dwtempx_short = dwtempx[iframe_left+p]
             		dwtempy_short = dwtempy[iframe_left+p]      
        				wavestats/Q dwtempx_short
                  if ( abs(dwtempx[iframe]-dwtempx[iframe-1]) > 3 * V_sdev ) 
                       dwtempx[iframe]=V_avg  
                   endif
  


         			wavestats/Q dwtempy_short
             
                  if ( abs(dwtempy[iframe]-dwtempy[iframe-1]) > 3 * V_sdev ) 
                       dwtempy[iframe]=V_avg  
                   endif
              endfor
...
//wave datawave
NVAR MicronsPerPixel  //12/10/21 DHR
variable nmperpixel = 1000 *MicronsPerPixel  //convert to nm per pixel
...
for ( ifreq = 0 ; ifreq<numofavg; ifreq+=1)
       ...
       temptracewave = temptracewave*nmperpixel
...
Prompt bg_thresh,"Enter threshold for background posts"
Prompt recalc_msd_flag, "Type 0 for recalculate MSD"

DoPrompt "parameters" framenum, slopethresh,bg_thresh, recalc_msd_flag
cellmotdetect(fitres_ed, 500, slopethresh,bg_thresh,recalc_msd_flag)
...
variable force_thresh_high = 5
variable force_thresh_low = 2
prompt kspring, "enter micropost spring constang (in nN/um)"
prompt force_thresh_high, "enter threshold above which considered as stress fiber posts (in nN)"
prompt force_thresh_low, "enter threshold below whici considered as cortical posts (in nN)"
doprompt "parameter", kspring,force_thresh_high,force_thresh_low
wave fitres_ed
...
findinteriorposts_dis(fitres_ed, linmaskval_intercept,kspring,force_thresh_high,force_thresh_low)
...
for (ipost = 0; ipost<npost; ipost+=1)
      ...
      CurveFit/Q /W=2/NTHR=0 line  datawmsd_log[50,100]  /X=msdxwave_seglog /D 
      beginslope[ipost] = w_coef[1]
      beginmag[ipost] = begintempmsd[100]
...
NVAR MicronsPerPixel  //12/10/21 DHR
variable pratio=MicronsPerPixel
...
function findinteriorposts_dis(datawave,linmaskval_intercept,kspring,forcethresh,forcethreshl)
...
fitres_ed_sub = (datawave[p][q][r] - zeropos[p][q][0])*pratio
variable V_max, V_min
variable noll     //number of points below threshold
variable pthresh= 16000    //number of points below threshold that forms plateau


make/o/n=(npost)  pforce_avg,pforce_angle
make/o/n=(npost, movielength) pforce
make/o/n=(npost,2) pforce_plot
...
for(ipost = 0; ipost<npost; ipost+=1)
       ...
       pforce_angle[ipost] = -angle(fitres_ed_sub[ipost][0][0],fitres_ed_sub[ipost][1][0])
//modified categorization into always highly deflected, alwayls weakly deflected, ones that have transition
      for(i = 0; i<movielength; i+=1)
             if (sforce[i]>forcethreshl/kspring && V_max>forcethresh/kspring)
                  noll+=1
...
pforce_plot[][0] = pforce_avg[p]
pforce_plot[][1] = pforce_angle[p]
...
function findpara(Rmin, Rmax, msdwave_diff)
variable Rmin, Rmax
wave MSDwave_diff
...
make/o/n=(npost)  msd_slope_mean=0, msd_slope_para=0, msd_mag_para=0, msd_slope_err=0
...
for(ipost = 0; ipost<npost; ipost+=1)
      ...
      wavestats/q/R=(Rmin,Rmax)   msd_diff_temp
         ...
         msd_slope_mean[ipost] = mean(msd_diff_temp,Rmin,Rmax)
...
//12/7/21  WILL HAVE TO BE EDITED FOR IGOR9 MOVIE HANDLING!  
variable posttype      // 1 for low traction force posts, 6 for high traction force posts
wave datawave     
...
for (i=0;i<nseg;i+=1)
     disptempx = datawave[postid][0][p+i*seglength]*nmperpixel
     disptempy = datawave[postid][1][p+i*seglength]*nmperpixel
