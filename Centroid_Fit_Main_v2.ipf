...
// 1/13/2012 Craig- Changed to updateGfit button (changegfit proc).  While this change was made for gaussian fitting analysis of phase images, 
                                     //the functionality may find use with fluoresence images as well.  When performing, the user has the option
                                     //to draw a shape around the post of interest, which results in the entire rest of the image being set to zero.  This keeps any bright signal
                                     // located near the post from interefering with the gaussian fit.   This is done for a specific post, OR for all cell posts in sequence.
...
//10/15/08 working on  making better initial guesses  wave zuza, passed into initguesswave
...
// 6/28/06  Modifications begun to bring Gfit up to date to parallel PSFfit 
 ...
 //                    CCD errors  option added to maintain compatibility
...
// 7/10/06 Updated to include errorbars and (incomplete) option for CCD errors
//5/31/06  FitBaseG() modified to save full results of gaussian basefit used for interpolation
//                  LoadBasefitG() now loads files used in interp. base fit
//                 ExamineBottomFitG() modified for interp (grid method)
//                  ChangeBottomFitG(), ExamineGfit(), ChangeGfit() adapted for G
...
#include ":FitAllPostsGerrChkHexM1.531"
#include  ":HelperRoutinesM2"   //9/7/21 DHR updated for Igor9
#include  ":GFitForBottom"
#include  ":DefineArray07M"   //9/13/21 DHR updated
#include  ":ExamineFitGM1.12"
#include  ":PlotAlot5.5M"  
#include  ":MakeTime3"
#include ":ViewGFitErrors"
#include ":ModernClicking04"  //9/16/21 DHR updated
#include ":FindShifts02"
#include ":MakeMaskGUI04M"  // 9/9/21 modified by YS
#include ":Correlation"
#include ":CalcMSD2M"   //9/7/21  DHR updated this file
#include ":Digital lock in1_12"
#include ":Histogram1_23temp"
#include ":activestress calculation1_33"
#include ":Levyanalysis_v1_5"
#include ":calccrosscorrelation2"   // 9/7/21 DHR updated
#include ":Frequency Dependence1_6"
...
String/G InGuess_String="InGuess_"+RootString
...
// 6/28/06
Variable/G CCDFlag = 0
Variable/G CCDReadNoise =13  // in electrons for UPenn Hammamatsu
Variable/G CCDeperADU   =3.2  // for UPenn 
...
// 12/10/13 -anything with "M" suffix changed for movie input data
	...
	Button GFitAll, pos={10,130}, size={130,20}, title="GFit All Posts", proc=GFitAll 
	Button Calcshift, pos={150,130}, size={110,20}, title="Calc_bgshift",proc=calcbgshift
//	Button GuideFitLoad, pos={170,220}, size={80,20}, title="Load Guide Fit", proc=GuideFitLoad  //load previous fit to guide posts                         

	
	//adeded 12/12/13  Yu Shi

	Button PlotMSD, pos={10,160}, size={130,20},title="MSD", proc=PlotMSD
//	Button heatmap, pos={150, 160}, size={110,20}, title= "HeatMap", proc=calcheatmap
	Button MSDmap, pos={150, 160},size={110,20}, title="MSDMap",proc=calcMSDheatmap
	Button MSDstatscalc, pos={10,190}, size={130,20}, title="MSDstats", proc=calcMSDstats
	Button  CELLMASK, pos={10,220},size={130,20}, proc = gencellmask, title="Make Cell Mask"
	Button  FORCEBIFURCATION, pos = {150,220}, size = {110,20}, proc = bifcell, title = "Force Bifurcation"
	
	
	Button CalcPostCenters, pos={150,190}, size={110,20}, title="Calc. Grid Center", proc=CalcPostCentersAndShifts // 
	...
	Button CONgfitall, pos = {10,310}, size = {110,20}, title = "con GfitAllPosts", proc = COUNTGFitAll
...
string CtrlName //amplitudes, backgrounds, etc.       //OUTPUT :  constantswave to hold various useful stuff
	Variable GridFlag = 1
	DefineArrayM(GridFlag)              
end           

// --------------GFitAll ------------
// 3/24/07 
// New - Gfits all posts, using intial shifts only to set initial fit conditions
...
//    Prompt autorefitflg, "Set = 1 for autorefit (fits each cell post 2nd time with smaller kfitrange)"
...
//    NVAR CCDflag // 6/28/06
    ...
    LoadWave/O/P=ToPractice InGuess_String+".ibw"; Duplicate/O $InGuess_String IGS
    LoadWave/O/P=ToPractice ShiftString+".ibw"
    Duplicate/O $AttributeString ASTr
 	
//    ControlInfo CCDErrors   //6/28/06
//    if (V_Value == 1)
...
// 	Set up Initial guesses
    Make/O/N=7 ZuZa=1                                 
    ZuZa[0]=IGS[0]; ZuZa[1]=IGS[1]       /// bkgnd and amplitude
    ZuZa[3] = 8  // Sigx
    ZuZa[5] = 8   //Sigy
    ZuZa[6] = 0.01  // Corr
//	ZuZa[0]=IGS[0]; ZuZa[1]=IGS[1]/11; ZuZa[2]=AveRad        ///Zhis may be subject to variations

//function GfitAllPosts(f1,f2,XTop,YTop,XPosts,YPosts,zmin,zmax,autorefit,maskwave,shiftwave,InitGuessWave,FRwave,FEwave,pathstr)
   //GfitAllPosts(f1,f2,ASTr[0],ASTr[1],ASTr[3],ASTr[4],ASTr[6],ASTr[7],autorefitflg,$MaskString,$ShiftString,zuza, ksRhodFileRoot,maskstring) //for use with more modern code
   centroidgfit(f1,f2,ASTr[0],ASTr[1],ASTr[3],ASTr[4],ASTr[6],ASTr[7],$MaskString,$ShiftString,zuza) //for back compatible code
	Duplicate/O FitResults $FitRes_String
	Duplicate/O FitErrors $FitErr_String
...
////-------------------------LoadGFit() --------------------------------------------------                        
function LoadGFit(ctrlName): ButtonControl 
string CtrlName  
...
///------------------------------------------------------
Function ViewGFitErrors(ctrlName):ButtonControl
string CtrlName
...
string p1string,p2string
    ...
    variable f1=0,f2=1,param1=0
    variable posti=0,postf
    f2 = dimsize(FitErrors,2) -1
...
Prompt p1string,"Variable to View",  popup "Bkgnd;Ampl;X0;SigX;Y0;SigY;Corr"
DoPrompt "View G Fit Errors", f1,f2,posti,postf,p1string


if (V_flag==1)	
		return 0
endif
if (stringmatch(p1string, "Background")==1)
       param1=kFbkgnd
elseif (stringmatch(p1string, "Ampl")==1)
       param1=kFAmpl
elseif (stringmatch(p1string, "X0")==1)
	param1=kFX0
elseif (stringmatch(p1string, "SigX")==1)
	 param1=kFSigX
elseif (stringmatch(p1string, "Y0")==1)
       param1=kFY0
elseif (stringmatch(p1string, "SigY")==1)
       param1=kFSigY
elseif (stringmatch(p1string, "Corr") == 1)
        param1 = kFCorr
endif

ViewErrors(f1,f2,posti,postf,param1,FitErrors)
...
string CtrlName   
	...
	if (V_Flag==1)
 	...
 	NVAR CCDflag // 6/28/06
	SVAR MaskString, FitRes_String, FitErr_String, ShiftString      
	SVAR  InGuess_String, AttributeString,FitBottom, FitErrBottom
...
//       calcShiftsGrelF1(f1,f2,ASTr[3],ASTr[4],$MaskString,$ShiftString,FitResults,FitErrors)
       calcShiftsGrelF1rot(f1,f2,ASTr[3],ASTr[4],$MaskString,$ShiftString,FitResults)

       
       Duplicate/O ZeroPos $FitBottom
       Duplicate/O ZeroPosErrors $FitErrBottom
	...
	Save/O/P=ToPractice $ShiftString as ShiftString+".ibw"  
       ...
       // *****Craig edit 8/16/11***********************************************************
      //  This calculation determines average lattice constant using the corners of main ROI
      //  This is only for determining stretch magnitude, which must be calculated from the "baseline" images
      //  Since, as of this 8/16/11, each stretch image is analyzed individually, this routine is only for a single frame
      variable avgXspacingTop
      variable avgYspacingLeft
      variable avgXspacingBot
      variable avgYspacingRight
      variable avgXspacingPix, avgYspacingPix,avgXspacing, avgYspacing,avgTotspacing
      //changed for centroid method 
      avgXspacingTop = (fitresults[(AStr[3]-1)*astr[4]][0] - fitresults[0][0])/(astr[3]-1)
      avgXspacingBot = (fitresults[(astr[3])*astr[4]][0] - fitresults[(astr[4]-1)][0])/(astr[3] - 1)
      avgXspacingPix = (avgXspacingTop + avgXspacingBot)/2
      
      
      avgYspacingLeft = (fitresults[(astr[4]-1)][1] - fitresults[0][1])/((astr[4]-1)/2)
      avgYspacingRight = (fitresults[(astr[3]*astr[4])][1] - fitresults[(astr[3]-1)*astr[4]][1])/((astr[4]-1)/2)
      avgYspacingPix = (avgYspacingLeft + avgYspacingRight) /2
     
     //convert to microns, assuming 60x objective = 37.2 pixels per 4 micron spacing = 9.3 pixels per micron
//     avgXspacing = avgXspacingPix/9.3
//     avgYspacing = (avgYspacingPix/sqrt(3))/9.3
//     avgTotspacing = (avgXspacing+avgYspacing)/2

     //convert to microns, assuming 40x objective = 24 pixels per 4 micron spacing = 6 pixels per micron
     avgXspacing = avgXspacingPix/6
     avgYspacing = (avgYspacingPix/sqrt(3))/6
     avgTotspacing = (avgXspacing+avgYspacing)/2
     
     Print "Average horizontal spacing is",avgXspacing,"microns"  
     Print "Average vertical spacing is",avgYspacing,"microns"
     Print "**** Avergae spacing is",avgTotspacing,"microns ****"
     //**********************************************************************************      
...
//-------------------------------------------------
//ExamineGfit()
//2/12/09 CMK commented this out, attempting GUI examine
// 7/7/06 temporary stub added - not ready for CCD errors yet
// 5/31/06 adapted for Gfitting
//  3/7/06 Notebook capability added
function ExamineGfit(ctrlName):ButtonControl
string CtrlName
	SVAR MaskString, FitRes_String, FitErr_String, RootString, AttributeString
      NVAR CCDflag   //6/28/06
	Variable npost=1, printflag=0, f1=0, f2=1,notebookflag =1
	Prompt npost, "Type in post number (-1 for all of them)"
	Prompt printflag, "Type to 1 print results"
	Prompt notebookflag, "Type to 1 create notebook"
	Prompt f1, "Starting file"
	Prompt f2, "End file"
	DoPrompt "Type in the info", npost, f1, f2, printflag,notebookflag
	if (V_flag==1)
		return 0
	endif
	...
	// temporary - 6/28/06
	if (CCDflag == 1)
	    Print "ExamineGfit not implemented for CCD errors"
	    return(0)
	endif

	//LoadWave/O/P=ToPractice "PSFInfo_"+RootString+".ibw"	  // Not needed for GFit
	LoadWave/O/P=ToPractice AttributeString+".ibw"
 	Duplicate/O $AttributeString ASTr
	ExamineFitG1post(npost,f1,f2,printflag,notebookflag,$MaskString,$FitRes_String,$FitErr_String,ASTr)
	Variable/G PInd=npost, F1_Ind=f1, F2_Ind=f2
end
///------------------------------------------------------------------
...
//7/10/06 Changes made to ROI selection in Gaussian Function. V3 variable replaced with v2
//7/7/06:  stub added:  not implemented for CCD errors yet

// 5/31/06 adapted for Gfitting
function ChangeGfit(ctrlName): ButtonControl
string CtrlName
	wave ASTR,W_Sigma
	NVAR PInd, F1_Ind, F2_Ind 
      NVAR CCDflag   //6/28/06
	Variable npost=0, f1=0, f2=0, ifile, AngleofRotation=0
	...
	Variable specificpostflag=0
	Prompt specificpostflag, "1=specific post,  0=all"
	Prompt f1, "Start File #"
	Prompt f2, "End File #"
	DoPrompt "File info",specificpostflag, f1, f2
	if (V_flag==1)
		return 0 
	endif 
...
//	DoPrompt "Multiplication factors ROI, Lattice "+num2str(ASTR[5]), MulLTX, MulLTY, MulRBX, MulRBY
	...
	wave linmaskval,linmaskxorig,linmaskyorig,fitresultsbase
	//******************************

	
	for (ifile=f1; ifile<=f2; ifile+=1)
		for(npost =0;npost<totalnumposts;npost+=1)
		if(specificpostflag==1)
			Prompt npost, "Which cell post do you want to fix?"
			DoPrompt "Post select",npost
		endif
		if(linmaskval[npost]==1)
		//----------Added
		  //sprintf s1, "%sRHOD%02d.tif",pathstr,ifile
             // sprintf s1, "RHOD%02d.tif", ifile
        	 sprintf s1, "%s%03d.tif", ksRhodFileRoot,ifile   // 4/3/07
     		  AngleOfRotation = shiftwave[ifile][7]
 		 // 2/17/06 DHR changed
		...
		ModifyGraph/w=rotatedimage expand=.6
		//**************
		//MatrixFilter/N=(3)/P=1 median rotatedorig1
	      //MatrixFilter/N=(3)/P=1 gauss rotatedorig1
	//	MatrixFilter/N=(3)/P=3 avg rotatedorig1
	     // MatrixFilter/N=(3)/P=5 min rotatedorig1
	//	MatrixFilter/N=(3)/P=3 avg rotatedorig1
		
		//using preprocessing from Yu Shi
	     MatrixFilter/N=(3)/P=1 point rotatedorig1
	     MatrixFilter/N=(9)/P=1 gauss rotatedorig1
		
		v2 = linmaskxorig[npost]
		v4 = linmaskyorig[npost]
		//***************
       ...
       ///------------------------------------
		...
		//*******************************************************************************************************************************************************************************
		Variable drawflag=0,goodflag=1
	
		//the following overplots the current center point
		Make/o/n=1 currentfitx,currentfity
		Make/o/n=1 Basex,Basey
		currentfitx[0] = FRS[npost][kFx0][ifile]
		currentfity [0]= FRS[npost][kFY0][ifile]
		Basex[0] = fitresultsbase[npost][kfx0][ifile]
		Basey[0] = fitresultsbase[npost][kfy0][ifile]
		//Appendtograph/T basey vs basex
		//Modifygraph mode(basey)=3,marker(basey)=19,msize(basey)=6,rgb(basey)=(0,65535,0)
		Appendtograph/T currentfity vs currentfitx
		ModifyGraph mode(currentfity)=3,marker(currentfity)=19,msize(currentfity)=6
//		Appendtograph/T linmaskyorig vs linmaskxorig
//		ModifyGraph mode(linmaskyorig)=3,marker(linmaskyorig)=1,msize(linmaskyorig)=6
		DoUpdate/w=rotatedimage
		
		Prompt drawflag, "1=yes,0=no"
		DoPrompt "Do you want to refit and draw the roi?" drawflag
		
		 if(drawflag==1)     
		 	Fctr = .6
		      Dowindow/K DrawROIpanel
		      Newpanel/k=1/N=drawroipanel/W=(1050,500,1200,600)
		  	...
		  	Button DrawIt , pos = {B1pos, line2}, size = {100,20}, title = "LetsDrawit", proc = Drawbutton 
		  	
		      pauseforuser drawroipanel,rotatedimage
			...
			if(nodrawflag[0]==1)
	            ...
	            MulLTX=Fctr; MulLTY=Fctr; MulRBX=Fctr; MulRBY=Fctr
			//NOTE: Astr[5] is lattice ct. in pixels
	            SetAxis/R left (v4+ASTR[5]+ShiftWave[ifile][11]),(v4-ASTR[5]+ShiftWave[ifile][11]);  // Fixed ROI, no shift!!! //changed this to the shifts put in by hand for cases when the calculated shift is wrong, due to a misfit
		      SetAxis top (v2-ASTR[5]+ShiftWave[ifile][10]),(v2+ASTR[5]+ShiftWave[ifile][10])
		      TextBox/C/N=text0/A=RT/X=1.00/Y=1.00 num2str(ifile)
		      TextBox/C/N=text1/A=RT/X=5.00/Y=1.00 num2str(npost) 
		      CurveFit/Q Gauss2d  kwCWave=Inits RotatedOrig1[(v2-ASTR[5]*MulLTX+shiftwave[ifile][10]),(v2+MulRBX*ASTR[5])+shiftwave[ifile][10]][(v4-ASTR[5]*MulLTY+shiftwave[ifile][11]),(v4+ASTR[5]*MulRBY+shiftwave[ifile][11])] /D 
		  endif
	...
	//***********	*************************************************************************************************************************************************************
		...
		//endif
			...
			//******************************************
			If(drawflag==1)
			Prompt goodflag, "1=yes,0=no"
 			Doprompt "Are you happy with the refit? If not, lets try again.", goodflag
 			if(goodflag==0)
 				npost = npost-1
 				Fctr=.6
 			endif
 			if(specificpostflag==1&&goodflag==1)
 				Killwindow rotatedimage
 				//break
 			endif
 			endif
			//******************************************
...
// 4/14/10  refits ALL posts in ALL frames

//7/10/06 Changes made to ROI selection in Gaussian Function. V3 variable replaced with v2
//7/7/06:  stub added:  not implemented for CCD errors yet

// 5/31/06 adapted for Gfitting
function ChangeGfitAll(ctrlName): ButtonControl
string CtrlName
	wave ASTR,W_Sigma
	NVAR PInd, F1_Ind, F2_Ind 
      NVAR CCDflag   //6/28/06
	Variable npost=PInd, f1=F1_Ind, f2=F2_Ind, ifile, AngleofRotation=0,f3=-1
	SVAR FitRes_String, FitErr_String, ShiftString
	SVAR MaskString,  AttributeString
	Duplicate/O $FitRes_String FRS
	Duplicate/O $FitErr_String FRS_Err
	...
	Prompt f2, "End File #"
	//****
	Prompt f3, "Post number if you don't want to do them all, -1 for all"
	//****
	DoPrompt "File info",  f1, f2, f3
	if (V_flag==1)
		return 0 
	endif 
	...
	if (CCDflag == 1)
     ...
     variable i,j
     variable nxpost1 = ASTR[3]
     variable nypost1 = ASTR[4]
     variable totalposts = nxpost1*nypost1  //compute total number of posts
     for (npost = 0 ; npost <totalposts; npost +=1)
     	//********
     	 if(f3 != -1)
     	   npost = f3
     	  endif
         ...
         i = floor(npost/nypost1)
         j = mod(npost,nypost1)
 		if(maskwave[i][j]==Kcell || maskwave[i][j]==kempty )	   
...
variable posti=0,postf=1,param1=3,param2=4,shiftflag = 1,printflag=0, notebookflag = 1, timeflag = 0,cellpostflag=0, bgshiftflag=1, spikeflag=1
//string p1string="X",p2string="Y"
...
String LastPost="Last post (Perhaps "+num2str(AStr[3]*Astr[4]-1)+" ?)"

Prompt posti,"First post"
Prompt postf, LastPost
//Prompt param1, "Param 1", popup "2;3;0"
//Prompt param2, "Param 2" , popup "4;7;1"
//Prompt p1string, "2 - radius, 3 - X coord, 0-Background"
//Prompt p2string, "4 for Y coord, 7 for Chisq, 1- Amplitude" 
Prompt p1string,"Print in Left Column",  popup "X;Background;SigX;Corr"
Prompt p2string,"Print in Right Column",  popup "Y;Amplitude;SigY;ChiSq"


Prompt notebookflag, "type 1 to make notebook"
//Prompt shiftflag, "0 for GridEach,  1 for Shift"
Prompt shiftflag, "0 for GridEach,  1 for absolute"  // 4/23/10
//Prompt printflag, "type 1 to print directly"
Prompt cellpostflag, "Type 1 to plot only cell posts"
Prompt timeflag, "Type 1 to plot vs timewave"
Prompt bgshiftflag, "Type 0 to plot raw deflection."
Prompt framerate, "type framerate"
prompt spikeflag, "type 1 to remove spikes"
//DoPrompt "Input all the stuff necessary", posti,postf,param1,param2,p1string,p2string,shiftflag,printflag, notebookflag, timeflag
DoPrompt "Input all the stuff necessary", posti,postf,p1string,p2string,shiftflag,cellpostflag, notebookflag, timeflag, bgshiftflag, framerate
DoPrompt  "Input all the stuff necessary", spikeflag    
...
if (stringmatch(p1string, "X")==1)
	param1=kFX0
elseif (stringmatch(p1string, "Background")==1)
       param1=kFbkgnd
elseif (stringmatch(p1string, "SigX")==1)
	 param1=kFSigX
elseif (stringmatch(p1string, "Corr") == 1)
...
if (stringmatch(p2string, "Y")==1)
	param2=kFY0
elseif (stringmatch(p2string, "Amplitude")==1)
       param2=kFAmpl
elseif (stringmatch(p2string, "SigY") == 1)
       param2 = kFSigY
elseif (stringmatch(p2string, "ChiSq")==1)
       param2=kFChisqG
endif
...
if (shiftflag == 0)  // 3/25/07 compute displacements
...
elseif (shiftflag == 1)  
//     ApplyShiftsPSF2(f1,f2,ASTr[3],ASTr[4],$ShiftString,fitres_plotAlot,fiterr_plotAlot)  //6/02/06 - eb added
//4/23/10     ApplyShiftsG(f1,f2,ASTr[3],ASTr[4],$ShiftString,fitres_plotAlot,fiterr_plotAlot)  //6/02/06 - eb added
endif

//PlotALot3EB(posti,postf,param1,param2,p1string,p2string,fitres_plotAlot,$FitErr_String,printflag,notebookflag)
//7/7/06 
PlotALot4EB(posti,postf,param1,param2,p1string,p2string,ASTr,$MaskString,fitres_plotAlot,$bgshiftstring,cellpostflag,notebookflag,timeflag,bgshiftflag, framerate, spikeflag)  // //6/02/06 - eb added
end
///----------------------------------------------------
// MakeItQuiverG()
...
function plotMSD(ctrlName): ButtonControl 
string ctrlName
SVAR FitRes_String, AttributeString,MaskString, bgshiftstring
variable posti=0,postf=1,param1=3,param2=4,shiftflag = 0,printflag=0, notebookflag = 1, timeflag = 0,cellpostflag=0,maxt=0.1,slopet=1
variable filterflag=1, freqflag = 0 //, freq =1 
variable f1,f2
variable framerate=10
variable spikeflag=1
variable directionflag = 1     // 1 for calculation in local vector coordinate
...
string fitname, bgname, fitaffix, MSDname
...
Prompt timeflag, "Type 1 to plot vs timewave"
//DoPrompt "Input all the stuff necessary", posti,postf,param1,param2,p1string,p2string,shiftflag,printflag, notebookflag, timeflag
//Prompt maxt, "type maxMSD threshold"
//Prompt slopet, "type slope threshold"
Prompt filterflag, "type 1 for applying notch filter"
//Prompt freq, "frequency for filtering"
Prompt framerate, "type framerate"
Prompt spikeflag, " type 1 to remove spikes"
Prompt directionflag , "type 1 to plot in local vector coordinate"
Prompt freqflag, "type 1 to do frquency dependence analysis"
DoPrompt "Input all the stuff necessary", posti,postf, cellpostflag, notebookflag, timeflag, filterflag, framerate, spikeflag, directionflag, freqflag
...
variable MSDflag=1
   ...
   prompt MSDflag, " type 0 to recalculate MSD"
   doprompt " input all the stuff necessary", MSDflag
variable i
...
f1 = dimoffset($fitname,2)
f2 = dimsize($fitname,2) + dimoffset($fitname,2) 
          

duplicate/O $fitname fitres_plotalot    

if (MSDflag==0)
   calculateMSD(fitres_plotalot, $bgname, Astr, $Maskstring, cellpostflag, framerate, spikeflag, filterflag, 0)
//    MSDflag=1
    endif
    
    MSDname = "MSD" + fitaffix
    duplicate/o MSD $MSDname
...
if (MSDflag==0)
   calculateMSD(fitres_plotalot, $bgshiftstring, Astr, $Maskstring, cellpostflag, framerate, spikeflag, filterflag, freq[i])
//    MSDflag=1
//   calculateMSD_vshort(fitres_ed_s, Astr, $Maskstring, cellpostflag, framerate, spikeflag, filterflag, freq[i])
    endif
    
    
endif    

PlotMSDM(posti,postf,ASTr,$MaskString,fitres_plotalot,cellpostflag,notebookflag,timeflag,maxt, slopet,framerate,spikeflag, directionflag)
...
doprompt "input parameters",  framerate, freq,  reffreq, corstring, valstring
...
DFcalcDLI(ASTR, $fitres_string, $bgshiftstring, $maskstring, framerate, freq, reffreq, heatflag, initialshift)
...
//fitting posts center for all frequency
...
//function GfitAllPosts(f1,f2,XTop,YTop,XPosts,YPosts,zmin,zmax,autorefit,maskwave,shiftwave,InitGuessWave,FRwave,FEwave,pathstr)
   //GfitAllPosts(f1,f2,ASTr[0],ASTr[1],ASTr[3],ASTr[4],ASTr[6],ASTr[7],autorefitflg,$MaskString,$ShiftString,zuza, ksRhodFileRoot,maskstring) //for use with more modern code
   COUNTcentroidgfit(f1,f2,ASTr[0],ASTr[1],ASTr[3],ASTr[4],ASTr[6],ASTr[7],$MaskString,$ShiftString,zuza) //for back compatible code
//	Duplicate/O FitResults $FitRes_String
//	Duplicate/O FitErrors $FitErr_String
//	Save/O/P=ToPractice $FitRes_String  as FitRes_String+".ibw"	
//-	Save/O/P=ToPractice $FitErr_String as FitErr_String+".ibw"
