...
variable nmperpixel = 1000 *  MicronsPerPixel  //nm per pixel
...
//                wavestats/q/r=(lseg*k, lseg*k+lseg) histo_x
             wavestats/q histo_d
//               wavestats/q histo_y
            Lmax_cell[icell*nseg+k] =nmperpixel * max(v_max, abs(v_min))
        endfor    
        alpha_msd[icell] = msd_slope_max[ipost]
        Lmax_msd[icell] = nmperpixel*sqrt(mean(Histo_x_sq))
...
//       For(i=0; i < intervalnum; i+=laginterval)
       ...
       for(i=0; i<intervalnum; i+=1)
            ...
            Histo_d[i] = sqrt(Histo_x[i]^2+Histo_y[i]^2)
            Histo_x_sq[i] = (Histo_y[i]*nmperpixel)^2
