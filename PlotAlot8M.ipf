...
//12/9/21 See code below for identification of post types in masks
...
//    wave errorwave			   //  error bars (from Curvefit?)
   ...
   variable bgshiftflag      //added by YuShi   1/3/14
    ...
    for (ipost = posti; ipost <= postf; ipost = ipost + 1)
	    ...
	    if (maskwave[ipost] == 6)   //  high traction force posts after MSD and Force Bifurcation
	    	posttype= 6
	    elseif(maskwave[ipost] ==7)   // "not assigned" neither low nor high traction force posts after MSD and FB
	    	posttype = 7
	    elseif(maskwave[ipost]==-1 )  // From MSD analysis in MakeCellMask: disengaged for 1st 1/3 of trajectory , may be engaged later  
	    	posttype=-1
	    elseif(maskwave[ipost] ==-2 )   // From MSD analysis in MakeCellMask: disengaged for last 1/3 of trajectory , may be engaged earlier  
	    	posttype=-2
	    elseif(maskwave[ipost] ==1)   // cell post  ;  low traction force (cortical) after MSD and FB
	    	posttype = 1
	    elseif(maskwave[ipost] ==3)    // ignored post
	    	posttype = 3
	    elseif(maskwave[ipost] ==4)    // magnetic post  (no longer used in mask)
	    	posttype = 3 
	    elseif(maskwave[ipost] == 0)   // background post
	    	posttype = 0   
	    endif    
...
function dispfitres4EB(frwave,npost,nparam,pnamestr,swname,bgshiftwave,nplot,posttype,i,j,cellpostflag,bgshiftflag,framerate, spikeflag)
    ...
    variable pixratio = MicronsPerPixel   // microns per pixel
    variable dstart = 0
    ...
    if (spikeflag==1)
       wavestats/Q dwtemp
             ...
             for (iframe=woff+1; iframe<(wlen+woff); iframe+=1)  
                  if ( abs(dwtemp[iframe]-dwtemp[iframe-1]) > 3 * V_sdev ) 
//                     if (iframe==woff+1)
  ...
  //                   endif
   ...
   setscale/P x 0, (1/framerate),"",dwtemp
    ...
    // set up for different symbols based on post mask   Note that some are only set by MSD and Force Bifurcation analysis
    if (posttype == kGuide)
        psym = 19
    elseif (posttype == 1)     // cell post  ;  low traction force (cortical) after MSD and FB
        psym = 8
        gval = 50000
    elseif (posttype == 0)     // background post
        psym = 6
        bval = 0
        rval = 0
        gval = 0
    elseif (posttype == 3)    // ignored post
        psym = 16
        rval = 39321
        bval =  39321       
        gval = 39321
    elseif (posttype == 4)    // magnetic post  (no longer used in mask)
        psym = 21
        rval = 30000
        bval =  30000
     elseif (posttype == 6)     //  high traction force posts after MSD and FB
       bval =50000
    elseif (posttype == -1)    // From MSD analysis in MakeCellMask: disengaged for 1st 1/3 of trajectory , may be engaged later  
    rval = 65280
    gval = 43520
    elseif ( posttype ==-2)    // From MSD analysis in MakeCellMask: disengaged for last 1/3 of trajectory , may be engaged earlier  
    rval = 50000
//    gval = 25000
     elseif (posttype == 7)    // "not assigned" neither low nor high traction force posts after MSD and FB
     rval = 36864
     gval = 14592
     bval = 58880
    endif
